	...
	-->
		<div class="well well-small">
			...
			<div class="row-fluid">
				...
				<c:if test="${featureProduct.size() > 0 }">
					<ul class="thumbnails">
						<c:forEach var="item" items="${featureProduct }" varStatus="loop">
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool" href="product_details.html"
										title="add to cart"><span class="icon-search"></span>
							...
							<c:if
								test="${(loop.index + 1) % 3 == 0 || (loop.index + 1) == featureProduct.size() }">
					</ul>
					<c:if test="${(loop.index + 1)  < featureProduct.size()}">
						<ul class="thumbnails">
					</c:if>
				</c:if>
				</c:forEach>
